            ...
            if(isAuthenticated()) {
                ...
                return (
                    <span className="movie-actions-container">
                        <Button icon onClick={(e) => this.movieSaveAction(e, 'like')} tooltipLabel="Ajouter à vos favoris"><i className={favoriteClass} id={'js-like-' + this.props.movie.id}></i></Button>
                        <Button icon onClick={(e) => this.movieSaveAction(e, 'watch')}  tooltipLabel="Ajouter aux films déjà vus"><i className={watchedClass} id={'js-watch-' + this.props.movie.id}></i></Button>
                        <Button icon onClick={(e) => this.movieSaveAction(e, 'wish')} tooltipLabel="Ajouter à votre liste de film à voir"><i className={wishedClass} id={'js-wish-' + this.props.movie.id}>playlist_add</i></Button>
                        <Button icon onClick={(e) => this.props.openCollectionAddMovieModal(e, this.props.movie)} tooltipLabel="Ajouter à une collection"><i className={pinClass}  id={'js-pin-' + this.props.movie.id}></i></Button>
                    </span>
                )
            }
        }
